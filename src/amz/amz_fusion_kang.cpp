  ...
  if (this->yolo_locker == 1 && this->box_locker == 1)
  {
    ...
    for (const auto &Box : lidar_boxes)
    {
      ...
      if (Box.y >= 0) // 
      {
        ...
        cv::Mat newpos5(this->transformMat_left * pos5);
        cv::Mat newpos6(this->transformMat_left * pos6);
        cv::Mat newpos7(this->transformMat_left * pos7);
        cv::Mat newpos8(this->transformMat_left * pos8);
      ...
      {
        // 카메라 원점 xyz 좌표 (3,1)생성
        cv::Mat newpos9(this->transformMat_right * pos1); // 카메라 좌표로 변환한 것.
        cv::Mat newpos10(this->transformMat_right * pos2);
        cv::Mat newpos11(this->transformMat_right * pos3);
        cv::Mat newpos12(this->transformMat_right * pos4);
        cv::Mat newpos13(this->transformMat_right * pos5); // 카메라 좌표로 변환한 것.
        cv::Mat newpos14(this->transformMat_right * pos6);
        cv::Mat newpos15(this->transformMat_right * pos7);
        cv::Mat newpos16(this->transformMat_right * pos8);


        float x1 = (float)(newpos9.at<double>(0, 0) / newpos9.at<double>(2, 0));
        float y1 = (float)(newpos9.at<double>(1, 0) / newpos9.at<double>(2, 0) + 480);

        float x2 = (float)(newpos10.at<double>(0, 0) / newpos10.at<double>(2, 0));
        float y2 = (float)(newpos10.at<double>(1, 0) / newpos10.at<double>(2, 0) + 480);

        float x3 = (float)(newpos11.at<double>(0, 0) / newpos11.at<double>(2, 0));
        float y3 = (float)(newpos11.at<double>(1, 0) / newpos11.at<double>(2, 0) + 480);

        float x4 = (float)(newpos12.at<double>(0, 0) / newpos12.at<double>(2, 0));
        float y4 = (float)(newpos12.at<double>(1, 0) / newpos12.at<double>(2, 0) + 480);
        
        float x5 = (float)(newpos13.at<double>(0, 0) / newpos13.at<double>(2, 0));
        float y5 = (float)(newpos13.at<double>(1, 0) / newpos13.at<double>(2, 0) + 480);

        float x6 = (float)(newpos14.at<double>(0, 0) / newpos14.at<double>(2, 0));
        float y6 = (float)(newpos14.at<double>(1, 0) / newpos14.at<double>(2, 0) + 480);

        float x7 = (float)(newpos15.at<double>(0, 0) / newpos15.at<double>(2, 0));
        float y7 = (float)(newpos15.at<double>(1, 0) / newpos15.at<double>(2, 0) + 480);

        float x8 = (float)(newpos16.at<double>(0, 0) / newpos16.at<double>(2, 0));
        float y8 = (float)(newpos16.at<double>(1, 0) / newpos16.at<double>(2, 0) + 480);
...
///////////////////////////////////////
        // cout << "max_iou_index: " << max_iou_index << endl;
