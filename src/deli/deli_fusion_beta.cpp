...
public:
  ...
  : Node("projection_box")
  {
    ...
    // 결과 퍼블리시
    publish_cone_ = this->create_publisher<std_msgs::msg::Float64MultiArray>(
      "/coord_xy", 1);
...
void ImageLiDARFusion::YOLOCallback(const vision_msgs::msg::Detection2DArray::SharedPtr msg)
{
  if (msg->detections.size() != 0 && this->yolo_locker == 0)
  {
    ...
    for (int i = 0; i <msg->detections.size(); i++)
    {
      ...
      if(msg->detections[i].bbox.size_x / msg->detections[i].bbox.size_y > 0.65)           //0.4
      {
        mut_yolo.lock();
        Box_yolo box_yolo = 
        {
        ...
        // cout << "x_size" << msg->detections[i].bbox.size_x << endl;
        // cout << "y_size" << msg->detections[i].bbox.size_y << endl; 
        mut_yolo.unlock();
      }
    }
    // mut_yolo.unlock();
