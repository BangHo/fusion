  ...
  if(this->sync_locker == 0 && this->deli_flag == 1)
  {
    ...
    if (yolo_msg->detections.size() != 0)
    {
      ...
      for (int i = 0; i <yolo_msg->detections.size(); i++)
      {
        ...
        // cout << "x/y : " << yolo_msg->detections[i].bbox.size_x / yolo_msg->detections[i].bbox.size_y << endl;
        if(yolo_msg->detections[i].bbox.size_x / yolo_msg->detections[i].bbox.size_y > 0.68)
        {
          Box_yolo box_yolo = 
          {
    ...
    if (this->yolo_locker == 0 && this->box_locker == 0)
    {
      ...
      for ( int i = 0; i < yolo_boxes.size(); i++)
      {
        ...
        else if(coord_vector.size() == 1)
        {
          ...
          float x = coord_vector[0].x;
          float y = coord_vector[0].y;

          std::stringstream xx;
          std::stringstream yy;

          xx << std::fixed << std::setprecision(2) << x;
          yy << std::fixed << std::setprecision(2) << y;
          ...
          text.ns = "text";
          if (class_id == 1)     {text.text = "A1\nx:" + xx.str() + "m\ny:" + yy.str() + "m"; text.color.r = 1.0; text.color.g = 0.0; text.color.b = 0.0;}
          else if (class_id == 2){text.text = "A2\nx:" + xx.str() + "m\ny:" + yy.str() + "m"; text.color.r = 0.0; text.color.g = 1.0; text.color.b = 0.0;}
          else if (class_id == 3){text.text = "A3\nx:" + xx.str() + "m\ny:" + yy.str() + "m"; text.color.r = 0.5; text.color.g = 5.0; text.color.b = 0.5;}
          else if (class_id == 4){text.text = "B1\nx:" + xx.str() + "m\ny:" + yy.str() + "m"; text.color.r = 1.0; text.color.g = 1.0; text.color.b = 0.0;}
          else if (class_id == 5){text.text = "B2\nx:" + xx.str() + "m\ny:" + yy.str() + "m"; text.color.r = 1.0; text.color.g = 0.0; text.color.b = 1.0;}
          else if (class_id == 6){text.text = "B3\nx:" + xx.str() + "m\ny:" + yy.str() + "m"; text.color.r = 0.0; text.color.g = 1.0; text.color.b = 1.0;}
          else{text.text = "None";}
          text.id = i;
          ...
          text.pose.position.y = coord_vector[0].y - 1;
          text.lifetime.nanosec = 200000000;
          text_array.markers.push_back(text);

          visualization_msgs::msg::Marker node;
          node.header.frame_id = "velodyne";
          node.type = visualization_msgs::msg::Marker::SPHERE;
          if      (class_id == 1){node.color.r = 1.0; node.color.g = 0.0; node.color.b = 0.0;}
          else if (class_id == 2){node.color.r = 0.0; node.color.g = 1.0; node.color.b = 0.0;}
          else if (class_id == 3){node.color.r = 0.5; node.color.g = 5.0; node.color.b = 0.5;}
          else if (class_id == 4){node.color.r = 1.0; node.color.g = 1.0; node.color.b = 0.0;}
          else if (class_id == 5){node.color.r = 1.0; node.color.g = 0.0; node.color.b = 1.0;}
          else if (class_id == 6){node.color.r = 0.0; node.color.g = 1.0; node.color.b = 1.0;}
          node.id = i + 1;
          node.color.a = 1.0;
          node.scale.x = 0.5;
          node.scale.y = 0.5;
          ...
          node.pose.position.x = coord_vector[0].x;
          node.pose.position.y = coord_vector[0].y;
          node.lifetime.nanosec = 200000000;
          text_array.markers.push_back(node);
        ...
        else if (coord_vector.size() > 1)
        {
          ...
          float x = coord_vector[min_index].x;
          float y = coord_vector[min_index].y;
          ...
          text.pose.position.y = coord_vector[min_index].y - 1;
          ...
          node.action = visualization_msgs::msg::Marker::ADD;
          node.pose.orientation.w = 1.0;
          node.pose.position.x = coord_vector[min_index].x;
          node.pose.position.y = coord_vector[min_index].y;
          node.lifetime.nanosec = 100000000;
          text_array.markers.push_back(node);
        }
      }
